...
def sendImagesToWeb(): 
  ...
  CONSIDER = set(["person","sofa","bottle"])
  ...
  while True:
      ...
      # receive RPi name and frame from the RPi and acknowledg# the receipt
      (rpiName, frame) = imageHub.recv_image()
      if np.shape(frame)==(640,480,6):
          continue
      frame1=frame[:,:,[0,1,2]]
      frame2=frame[:,:,[3,4,5]]
      #frame3=frame[:,:,[6,7,8]]
      #frame4=frame[:,:,[9,10,11]]
      #frame5=frame[:,:,[12,13,14]]
      ...
      (h, w) = frame1.shape[:2]
      blob = cv2.dnn.blobFromImage(cv2.resize(frame1, (300, 300)),0.007843, (300, 300), 127.5)
      #blob = cv2.dnn.blobFromImage(frame1, 0.007843, (640,480), 127.5)
      ...
      # the prediction
          ...
          if confidence > args["confidence"]:
              ...
              if CLASSES[idx] in CONSIDER:
                  ...
                  cv2.rectangle(frame1, (startX, startY), (endX, endY), (255, 0, 0), 2)

          # draw the sending device name on the frame
          cv2.putText(frame1, rpiName, (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)

          # draw the object count on the frame1
          label = ", ".join("{}: {}".format(obj, count) for (obj, count) in objCount.items())
          cv2.putText(frame1, label, (10, h - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255,0), 2)

      # update the new frame in the frame dictionary
      #frameDict[rpiName] = [frame1,frame2]
      frame = np.concatenate([frame1,frame2],axis=1)
      frames = cv2.imencode('.jpg',frame)[1]

      cv2.imshow("all",frame)
      #cv2.imshow("object detection", frame1)
      #cv2.imshow("drowsy detection", frame2)
